...
public class Server implements Runnable {
	...
	private boolean validateRecord(Record record) {
		if (!(record.getDate().equals("") || record.getNurse().equals(""))) {
			String nurse = record.getNurse().replace(" ", "");
			if (db.findUser(nurse) instanceof Nurse) {
				return true;
			}
		}
		return false;
	...
	private void create(String name, ObjectOutputStream out, ObjectInputStream in)
			throws IOException, ClassNotFoundException {// added method to check
														// really acceptable
														// record
		ArrayList<Record> recordsTemp = db.getPatientRecords(name);
		String date = null;
		if (currentUser instanceof Doctor && recordsTemp != null) {
			...
			Record temp = new Record(currentUser.getName(), received.getNurse(), currentUser.getDivision(),
					received.getDate(), received.getMedicalData());
			if (validateRecord(temp)) {
	...
	private static ServerSocketFactory getServerSocketFactory(String type) {
		if (type.equals("TLS")) {
			...
			try { // set up key manager to perform server authentication
				...
				// password
				// (storepass)
				kmf.init(ks, "Server".toCharArray()); // certificate password
														// (keypass)
